                ...
                <form @submit.prevent="handleSubmit" class="space-y-4">
                    <div class="space-y-2">
                        <Label class="text-base font-semibold">Код подтверждения отправлен на вашу электронную почту
                            ******@gmail.com</Label>
                        <div class="relative">
                            <Input v-model="emailCode" type="text" placeholder="Введите код"
                                class="bg-[#071722] text-base !py-6 !px-4 pr-10"
                                :class="{ 'border-red-500': emailCode.trim() === '' }" required />
                            <Copy class="absolute right-4 top-1/2 -translate-y-1/2 cursor-pointer z-20" />
                        </div>
                        <p v-if="emailCode.trim() === ''" class="text-red-500 text-sm">Это поле обязательно</p>
                    </div>
                    <p class="text-sm font-thin text-right">Изменить <a href="#!" class="text-blue-500 underline"
                            @click="emit('show2fa')">настройки</a> аутентификации</p>
